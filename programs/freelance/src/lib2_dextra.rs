...
pub mod dextra {
    ...
    fn verify_owner_or_governance(
        owner: Pubkey,
        governance: Pubkey,
        signer: Pubkey,
    ) -> Result<()> {
        require!(
            owner == signer || governance == signer,
    ...
    fn calculate_reward(pool_id: u64, user_info: &UserInfo, pool: &Pool) -> Result<u64> {
        ...
        let mut total_time_reward = 0;
        let mut current_claimed = last_claimed;

        // Calculate rewards for each day
        let seconds_per_day: i64 = 86400;
        for timestamp in (start_timestamp..clock.unix_timestamp).step_by(seconds_per_day as usize) {
            ...
            let yield_amount = (amount as u128)
                ...
                .checked_mul(pool.last_apy as u128).unwrap()
                .checked_div((100 * 365 * 86400 * 100) as u128).unwrap();

            total_time_reward += (yield_amount as u64)
                .checked_mul(pool.last_rate).unwrap()
        ...
        // Add decimal adjustment for token precision differences
        let deposit_decimals = safe_decimals(&pool.deposit_token)?;
        let receive_decimals = safe_decimals(&pool.reward_token)?;
        total_time_reward = if receive_decimals >= deposit_decimals {
...
fn safe_decimals(token: &Pubkey) -> Result<u8> {
    // Default decimal places for SOL and SPL tokens
    const DEFAULT_DECIMALS: u8 = 9;

    // If token is system program (native SOL), return default decimals
    if token == &system_program::ID {
        return Ok(DEFAULT_DECIMALS);
    }

    // Try to get mint account info
    match Mint::get_account_info(token) {
        Ok(mint) => Ok(mint.decimals),
        Err(_) => Ok(DEFAULT_DECIMALS)
    }
}
    // Add missing events
    #[event]
...
// Add safe_decimals helper function
fn safe_decimals(token: &Pubkey) -> Result<u8> {
    match token {
        token if *token == Pubkey::default() => Ok(18),  // Default for native token
        _ => {
            let mint = Mint::try_from_slice(&token.to_bytes())?;
            Ok(mint.decimals)
        }
    }
}
...
// Add decimals field to Pool struct
#[account]
pub struct Pool {
    pub deposit_token: Pubkey,
    pub reward_token: Pubkey,
    ...
    pub rates: HashMap<i64, u64>,
    pub apys: HashMap<i64, u64>,
    pub decimals: u8,  // Add this field
}

// Add constant for REF_PERCENT
pub const REF_PERCENT: u64 = 200; // 2%

// Add safe_approve helper function
fn safe_approve(
...
// Add SafeApprove account validation structure
#[derive(Accounts)]
pub struct SafeApprove<'info> {
    #[account(mut)]
...
// Add safe_approve instruction
pub fn safe_approve_instruction(
    ctx: Context<SafeApprove>,
    amount: u64,
) -> Result<()> {
    safe_approve(
...
// Add these helper functions
    ...
    fn calculate_swap(pool_id: u64, amount: u64, direction: bool) -> Result<u64> {
        // Simple 1:1 swap implementation
        // You might want to implement more complex swap logic based on your requirements
        Ok(amount)
    }
    // Add safe_transfer helper function
fn safe_transfer(
    token_program: &Program<Token>,
...
// Add safe_transfer_from helper function
...
// Add SafeTransferFrom account validation structure
#[derive(Accounts)]
...
// Add safe_transfer_from instruction
pub fn safe_transfer_from_instruction(
    ctx: Context<SafeTransferFrom>,
    amount: u64,
) -> Result<()> {
    safe_transfer_from(
        &ctx.accounts.token_program,
...
// Add safe_transfer_sol helper function
fn safe_transfer_sol(
    to: &AccountInfo,
    from: &AccountInfo,
    amount: u64,
) -> Result<()> {
...
// Add SafeTransferSol account validation structure
#[derive(Accounts)]
pub struct SafeTransferSol<'info> {
    #[account(mut)]
...
// Add safe_transfer_sol instruction
pub fn safe_transfer_sol_instruction(
    ctx: Context<SafeTransferSol>,
    amount: u64,
) -> Result<()> {
    safe_transfer_sol(
        &ctx.accounts.to,
...
// Add SafeTransfer account validation structure
#[derive(Accounts)]
...
// Add safe_transfer instruction
pub fn safe_transfer_instruction(
